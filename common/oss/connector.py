...
class S3Connector:
    ...
    def read_parquet(self, s3_bucket: str, s3_key: str, output:str = 'pandas', **kwargs):
        ...
        binary_data = self.read_binary(s3_bucket, s3_key)
        if binary_data is None:
            return None
        
        if output == 'arrow':
            return pq.read_table(pa.BufferReader(binary_data), **kwargs)
        else:
            return pd.read_parquet(BytesIO(binary_data), **kwargs)
    ...
    def write_parquet(self, df, s3_bucket, s3_key, pq_options={}, metadata=None):
        ...
        return self.write_binary(df_buf, s3_bucket, s3_key, metadata=metadata)
